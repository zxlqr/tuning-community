...
const Profile = () => {
  ...
  const updatePrivacyMutation = useMutation({
    ...
    onError: (error) => {
      ...
      alert('Ошибка при обновлении настроек приватности')
    },
  })
          ...
          <p>
            ...
            {isOwnProfile && (
              <label className="privacy-toggle">
                <input
                  type="checkbox"
                  checked={currentUser?.is_name_private || false}
                  onChange={(e) => handlePrivacyChange('is_name_private', e.target.checked)}
                />
                <span>Скрыть от других</span>
              </label>
            )}
